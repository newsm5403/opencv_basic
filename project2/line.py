...
def canny(img, low_threshold, high_threshold):  # Canny 알고리즘 경계선을 검출한다.
...
def region_of_interest(img, vertices, color3=(255, 255, 255), color1=255):  # ROI 셋팅 원하는 대상을 따로 분리해서 처리 하기 위해 설정한다.

    mask = np.zeros_like(img)  # mask = img와 같은 크기의 빈 이미지

    if len(img.shape) > 2:  # Color 이미지(3채널)라면 : 채널이 3일 경우, 다색 이미지이다. 채널이 1일 경우 단색 이미지이다.
        color = color3
    else:  # 흑백 이미지(1채널)라면 :
        color = color1
...
canny_img = canny(blur_img, 70, 210)  # Canny edge 알고리즘을 통해 윤곽선 이미지 리턴
